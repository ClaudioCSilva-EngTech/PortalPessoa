...
class DesligadoService {
  ...
  async criarVagasAutomaticas(desligados, usuarioLogado = null) {
    try {
      ...
      for (const desligado of desligados) {
        try {
          const dadosVaga = this.mapearDesligadoParaVaga(desligado, usuarioValidado);
          
          // Verificar se já existe uma vaga para este desligado
          const vagaExistente = await Vaga.findOne({ codigo_vaga: dadosVaga.codigo_vaga });
          
          if (vagaExistente) {
            console.log(`Vaga ${dadosVaga.codigo_vaga} já existe, pulando...`);
            continue;
          }
          ...
          vagasCriadas.push(novaVaga.toObject());
          
        } catch (error) {
          console.error(`Erro ao criar vaga para desligado ${desligado.idContratado}:`, error);
          erros.push({
            desligado,
            erro: error.message
          });
  ...
  mapearDesligadoParaVaga(desligado, usuarioLogado) {
    ...
    return {
      ...
      detalhe_vaga: {
        ...
        motivoAfastamento: `Funcionário: ${desligado.nomeCompleto} (ID: ${desligado.idContratado})`,
